...
import { ShieldCheck, Server, AlertCircle, Zap } from 'lucide-react';
import { isDemoModeActive, setDemoMode, checkBackendHealthApi } from '../services/api';
  ...
  return (
    <header className="header-container bg-white border-b border-purple-100 shadow-sm sticky top-0 z-40">
      ...
      <div className="top-brand-bar px-4 py-2 border-b border-slate-100 flex items-center justify-between bg-slate-900 text-white">
        <div className="flex items-center gap-3">
          ...
          <div className="logo-badge flex items-center gap-2 font-bold text-lg text-purple-300">
            ...
            <span className="tracking-wider text-white font-outfit text-xl">
              Universal Bank <span className="text-xs font-normal bg-purple-900/60 text-purple-200 px-2 py-0.5 rounded border border-purple-700/50 uppercase tracking-widest">Parser</span>
            </span>
          </div>
          ...
          <nav className="hidden lg:flex items-center gap-1 text-xs">
            <button
              ...
              className={`px-3 py-1.5 rounded-t-md font-medium transition ${activeTab === 'parser' ? 'bg-purple-700 text-white shadow' : 'text-slate-300 hover:text-white hover:bg-slate-800'}`}
            >
              📊 Statement Parser
            </button>
            <button
              onClick={() => setActiveTab('corporate')}
              className={`px-3 py-1.5 rounded-t-md font-medium transition ${activeTab === 'corporate' ? 'bg-purple-700 text-white shadow' : 'text-slate-300 hover:text-white hover:bg-slate-800'}`}
            >
              🏛️ Dashboard
            </button>
            <button className="px-3 py-1.5 text-slate-400 hover:text-slate-200 transition">
              + Transactions
            </button>
            <button className="px-3 py-1.5 text-slate-400 hover:text-slate-200 transition">
              + Reports
        ...
        {/* Right Section: Security & Backend API Status */}
        <div className="flex items-center gap-3 text-xs">
          ...
          >
            ...
            <span>Backend: {isDemoModeActive() ? 'Demo Mode' : 'API Connected'}</span>
            <span className={`w-2 h-2 rounded-full ${isDemoModeActive() ? 'bg-amber-400 animate-pulse' : 'bg-emerald-400'}`} />
          </button>
        </div>
      </div>
      ...
      <div className="notice-banner bg-amber-50 border-b border-amber-200/70 px-4 py-1.5 text-xs text-amber-900 flex items-center justify-between">
        <div className="flex items-center gap-2 overflow-hidden">
          ...
          <span className="truncate">
            <strong>Security Notice:</strong> AI Multimodal Extraction operates strictly in temporary server RAM. No bank statement PDFs or transactional records are retained in any database storage.
          </span>
        </div>
